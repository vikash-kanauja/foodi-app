...
const Navbar = () => {
  ...
  // console.log(cart.items?.length); // Output: Vikash
  // let notification = cart?.items?.length;
  ...
  useEffect(() => {
    if (user && cart) {
      // Re-render when user or cart data changes
      setName(user.name);
      setNotification(cart.items.length);
    }
  }, [user, cart]);
